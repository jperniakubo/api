...
export class OnboardingController {
  ...
  getCodeVerification = async (
    request: Request,
    response: Response,
    nextOrError: NextFunction
  ) => {
    const data: IUser.UidParam = request.body;
    const content = await this.onboardingService.getCodeVerification(data);
    await this.complementResponse.returnData(response, nextOrError, content);
  };
  ...
  verifyCode = async (
    ...
    const data: IUser.UidCode = request.body;
    const content = await this.onboardingService.verifyCode(data);
    await this.complementResponse.returnData(response, nextOrError, content);
  };
